...
def main(opts):
    if opts.pw:
        ...
        conn_string = "dbname='%s' user=%s host='%s'" % (opts.dbname, opts.user,  opts.host)
    ...
    cursor.execute("select id from mail_mass_mailing_list where name = '%s'" % mlist_name)
    row = cursor.fetchone()
    if not row:
        print '*' * 80
    ...
    for line in emails:
        if line.find('@') > -1:
            xxxx validate email ..
            email = line.strip()
            s = "select id from mail_mass_mailing_contact where email = '%s' and list_id = %s" % (email, mailinglist_id)
            cursor.execute(s)
            row = cursor.fetchone()
            if not row:
                s = "INSERT INTO mail_mass_mailing_contact (email, list_id) values ('%s', %s)"  % (email, mailinglist_id)
                cursor.execute(s)
                conn.commit()
                print email
    print '-- done --'
...
if __name__ == '__main__':
    ...
    parser.add_option("-d", "--dbname",
                    action="store", dest="dbname", default='afbs',
    ...
    parser.add_option("-m", "--mailinglist",
                    action="store", dest="mailinglist",
                    help="mailinglist to add emails to")

    parser.add_option("-u", "--user",
                    action="store", dest="user", default='robert',
                    help="define user default odoo")

    parser.add_option("-p", "--pw",
                    action="store", dest="pw", default='',
